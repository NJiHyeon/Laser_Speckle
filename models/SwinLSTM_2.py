...
class SwinTransformerBlock(nn.Module):
    ...
    def forward(self, x, hx=None):
        ...
        return x, shifted_x #attn_windows?
    ...
    def forward(self, xt, hx):
        ...
        for index, layer in enumerate(self.layers):
            if index == 0:
                ...
                if index % 2 == 0:
                    x, attn = layer(outputs[-1], xt)
                    outputs.append(x)
                    attn_outputs.append(attn)

                if index % 2 == 1:
                    x, attn = layer(outputs[-1], None)
                    outputs.append(x)
                    attn_outputs.append(attn)

        return outputs[-1], attn_outputs
...
class ST(nn.Module):
    ...
    def forward(self, x, h):
        ...
        for index, layer in enumerate(self.layers):
            x, hidden_state, attn = layer(x, h[index])
            hidden_states.append(hidden_state)

        return hidden_states, x, attn


class Regressor(nn.Module):
    def __init__(self, patch_dim, num_patches, hidden_dim, num_classes, drop):
        super(Regressor, self).__init__()
        self.fc1 = nn.Linear(num_patches*patch_dim, hidden_dim)
        self.fc2 = nn.Linear(hidden_dim, int(hidden_dim/2))
        self.proj = nn.Linear(int(hidden_dim/2), num_classes)
...
class SwinLSTM(nn.Module):
    ...
    def __init__(self, img_size, patch_size, in_chans, embed_dim, depths,
                 ...
                 hidden_dim_fc, dropout_fc, output_n):
        super(SwinLSTM, self).__init__()

        self.ST = ST(img_size=img_size, patch_size=patch_size, in_chans=in_chans,
        ...
        self.regressor = Regressor(patch_dim=embed_dim, num_patches=self.patch_num, hidden_dim=hidden_dim_fc, num_classes=output_n, drop=dropout_fc)

    def forward(self, input, states):
        states_next, output, attention = self.ST(input, states)
        final_output = self.regressor(output)
        return states_next, final_output, attention