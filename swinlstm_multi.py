                    ...
                    for hd in hidden_dim_fc:
                        for sf in skip_frames:
                            for bs in batch_sizes:
                                ...
                                save_model_path = f'./swinlstm2/multi/weights/ed{ed}_ps{ps}_ws{ws}_dt{dt}_nh{nh}_hd{hd}_sf{sf}_bs{bs}/'
                                save_pred_path = f'./swinlstm2/multi/predictions/ed{ed}_ps{ps}_ws{ws}_dt{dt}_nh{nh}_hd{hd}_sf{sf}_bs{bs}/'
                                log_path = f'./swinlstm2/multi/log/ed{ed}_ps{ps}_ws{ws}_dt{dt}_nh{nh}_hd{hd}_sf{sf}_bs{bs}/'
                                for paths in [log_path, save_model_path, save_pred_path]:
                                    os.makedirs(paths, exist_ok=True)

                                print(f'Embed dim: {ed}, Patch size:{ps}, Window size:{ws}, depth: {dt}, num head:{nh}, Skip frame:{sf}, Batch size:{bs}')
                                ...
                                model = SwinLSTM(img_size=128, patch_size=ps, in_chans=1, embed_dim=ed, depths=dt,
                                                ...
                                                hidden_dim_fc=hd, dropout_fc=0.0, output_n=5).to(device)
                                ...
                                result.to_csv(f'finetune_result/swinlstm2_multi.csv', index=None)

                                true, pred = trainer.predict(model_name='SwinLSTM2',depths=dt)
                                true = true*iqr + median
                                pred = pred*iqr + median
                                with open(os.path.join(save_pred_path, f'swinlstm2_multi.pickle'), 'wb') as f:
