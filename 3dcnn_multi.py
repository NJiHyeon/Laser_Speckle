            ...
            for do in drop_p:
                for bs in batch_size:
                    for sf in skip_frames:
                        ...
                        save_model_path = f'./3dcnn/multi/weights/hd{hd}_ks{ks}_fc{fc}_do{do}_bs{bs}_sf{sf}/'
                        save_pred_path = f'./3dcnn/multi/predictions/hd{hd}_ks{ks}_fc{fc}_do{do}_bs{bs}_sf{sf}/'
                        log_path = f'./3dcnn/multi/log/hd{hd}_ks{ks}_fc{fc}_do{do}_bs{bs}_sf{sf}/'
                        ...
                        logger.info(f'hidden dim:{hd}, kernel size: {ks}, fc hidden: {fc}, dropout: {do}, batch size: {bs}')
                        ...
                        train_set, valid_set, test_set = CustomDataset(data_path, train_list, train_label, selected_frames, transform=transform, cnn3d=True), \
                                                        CustomDataset(data_path, val_list, val_label, selected_frames, transform=transform, cnn3d=True), \
                                                        CustomDataset(data_path, test_list, test_label, selected_frames, transform=transform, cnn3d=True)
                        ...
                        result.to_csv('finetune_result/3dcnn_multi.csv', index=None)

                        true, pred = trainer.predict()
                        true = true*iqr + median
                        pred = pred*iqr + median
                        with open(os.path.join(save_pred_path, '3dcnn_multi.pickle'), 'wb') as f:
